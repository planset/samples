...
#                Lib.dumpM(u)
    ...
    def conn(self):
        self.getSource()
        log.debug('connect to %s %s' %(self.ns, self.port))
    ...
    def req(self,*name,**args):
        ...
        if qtype == Type.AXFR and protocol != 'tcp':
            log.debug('Query type AXFR, protocol forced to TCP')
            protocol = 'tcp'
        ...
        for self.ns in server:
            try:
                ...
                try:
                    ...
                    self.conn()
                    if not self.async:
                        log.debug('not async send request')
                        self.s.send(self.request)
                        log.debug('receive udp replay')
