...
def dnslookup(name,qtype,server='',protocol=''):
    "convenience routine to return just answer data for any query type"
    exargs = {}
    if server: exargs['server'] = server
    if protocol: exargs['protocol'] = protocol

    if Base.defaults['server'] == []: Base.DiscoverNameServers()

    result = Base.DnsRequest(name=name, qtype=qtype).req(**exargs)
